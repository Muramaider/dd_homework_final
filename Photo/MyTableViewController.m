...
#define SHOW_PHOTOS_SEGUE_ID @"firstSegue"

@interface MyTableViewController () <TagsServiceDelegate, UITextFieldDelegate> {
    SearchingInfo *selectedContent;
    WebServiceTag *service;
}
...
- (void)viewDidLoad {
    ...
    service = [[WebServiceTag alloc] init];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField {
    if (textField == self.searchTextField) {
        if ([textField hasText]) {
            selectedContent = [[SearchingInfo alloc] initWithContent:textField.text type:ContentTypeText];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    selectedContent = [[SearchingInfo alloc] initWithContent:cell.textLabel.text type:ContentTypeTag];
    [self performSegueWithIdentifier:SHOW_PHOTOS_SEGUE_ID sender:self];
}
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    if ([segue.identifier isEqual:SHOW_PHOTOS_SEGUE_ID]) {
        MyImgCollectionViewController *dvc = (MyImgCollectionViewController *)segue.destinationViewController;
        dvc.selectedInfo= selectedContent;
    }
}
